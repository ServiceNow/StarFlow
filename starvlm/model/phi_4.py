...
class PhiInput(VLLocalInput):
    ...
    def to(self, *args, **kwargs) -> "PhiInput":
...
class PhiModel(VLLocalModel):
    def get_model_and_processor(
        ...
        for layer in model.model.layers:
            layer.self_attn.o_proj.merge(adapter_names=["vision"])
            del layer.self_attn.o_proj.lora_dropout.speech
            del layer.self_attn.o_proj.lora_dropout.vision
            del layer.self_attn.o_proj.lora_A.speech
            del layer.self_attn.o_proj.lora_A.vision
            del layer.self_attn.o_proj.lora_B.speech
            del layer.self_attn.o_proj.lora_B.vision
            layer.self_attn.qkv_proj.merge(adapter_names=["vision"])
            del layer.self_attn.qkv_proj.lora_dropout.speech
            del layer.self_attn.qkv_proj.lora_dropout.vision
            del layer.self_attn.qkv_proj.lora_A.speech
            del layer.self_attn.qkv_proj.lora_A.vision
            del layer.self_attn.qkv_proj.lora_B.speech
            del layer.self_attn.qkv_proj.lora_B.vision
            layer.mlp.gate_up_proj.merge(adapter_names=["vision"])
            del layer.mlp.gate_up_proj.lora_dropout.speech
            del layer.mlp.gate_up_proj.lora_dropout.vision
            del layer.mlp.gate_up_proj.lora_A.speech
            del layer.mlp.gate_up_proj.lora_A.vision
            del layer.mlp.gate_up_proj.lora_B.speech
            del layer.mlp.gate_up_proj.lora_B.vision
            layer.mlp.down_proj.merge(adapter_names=["vision"])
            del layer.mlp.down_proj.lora_dropout.speech
            del layer.mlp.down_proj.lora_dropout.vision
            del layer.mlp.down_proj.lora_A.speech
            del layer.mlp.down_proj.lora_A.vision
            del layer.mlp.down_proj.lora_B.speech
            del layer.mlp.down_proj.lora_B.vision
    ...
    def get_layer_classes(self) -> set[type]:
        ...
        for (
            ...
            layer_classes.add(layer.__class__)
        layer_classes.add(
            self.model.model.embed_tokens_extend.image_embed.img_processor.head.__class__
        )
    ...
    def collate_inner(self, phi_inputs: list[PhiInput]) -> PhiInput:
        input_ids = torch.nn.utils.rnn.pad_sequence(
            [phi_input.input_ids.squeeze(0) for phi_input in phi_inputs],
            batch_first=True,
            padding_value=self.processor.tokenizer.pad_token_id,
        )
        attention_mask = torch.nn.utils.rnn.pad_sequence(
            [phi_input.attention_mask.squeeze(0) for phi_input in phi_inputs],
            batch_first=True,
            padding_value=False,
        )
        input_image_embeds = torch.nn.utils.rnn.pad_sequence(
            [phi_input.input_image_embeds.squeeze(0) for phi_input in phi_inputs],
            batch_first=True,
            padding_value=0.0,
        )
        image_sizes = torch.cat([phi_input.image_sizes for phi_input in phi_inputs])
        image_attention_mask = torch.nn.utils.rnn.pad_sequence(
            [phi_input.image_attention_mask.squeeze(0) for phi_input in phi_inputs],
            batch_first=True,
            padding_value=False,
        )
        labels = torch.nn.utils.rnn.pad_sequence(
            [phi_input.labels.squeeze(0) for phi_input in phi_inputs],
            batch_first=True,
    ...
    def forward(self, phi_input: PhiInput) -> Tensor:
        loss = self.model(
            input_ids=phi_input.input_ids,
            attention_mask=phi_input.attention_mask,
            input_image_embeds=phi_input.input_image_embeds,
            image_sizes=phi_input.image_sizes,
            image_attention_mask=phi_input.image_attention_mask,
            labels=phi_input.labels,
            input_mode=0,
            return_dict=True,
            use_cache=False,
        ).loss
    ...
    def generate_inner(self, phi_input: PhiInput) -> str:
        ...
        output = self.processor.tokenizer.batch_decode(
            output_ids[:, phi_input.input_ids.shape[1] :],
