...
class GemmaInput(VLLocalInput):
    ...
    def pin_memory(self) -> "GemmaInput":
...
class GemmaModel(VLLocalModel):
    ...
    def get_layer_classes(self) -> set[type]:
        ...
        for layer in self.model.model.vision_tower.vision_model.encoder.layers:
            layer_classes.add(layer.__class__)
        return layer_classes

    def preprocess(self, vl_example: VLExample, for_generate: bool) -> GemmaInput:
        input_ids = []
        token_type_ids = None
        pixel_values = None
        ...
        for index, (query, annotation) in enumerate(
            ...
            if is_first_turn:
                ...
                token_type_ids = feature["token_type_ids"].to(torch.long)
        ...
        attention_mask = torch.full_like(input_ids, fill_value=1)
        token_type_ids = torch.cat(
            [
                token_type_ids,
                torch.full(
                    [1, max(input_ids.shape[1] - token_type_ids.shape[1], 0)],
                    fill_value=0,
                    dtype=torch.long,
                ),
            ],
    ...
    def collate_inner(self, gemma_inputs: list[GemmaInput]) -> GemmaInput:
        input_ids = torch.nn.utils.rnn.pad_sequence(
            [gemma_input.input_ids.squeeze(0) for gemma_input in gemma_inputs],
            batch_first=True,
            padding_value=self.processor.tokenizer.pad_token_id,
        )
        attention_mask = torch.nn.utils.rnn.pad_sequence(
            [gemma_input.attention_mask.squeeze(0) for gemma_input in gemma_inputs],
            batch_first=True,
            padding_value=0,
        )
        token_type_ids = torch.nn.utils.rnn.pad_sequence(
            [gemma_input.token_type_ids.squeeze(0) for gemma_input in gemma_inputs],
            batch_first=True,
            padding_value=0,
        )
        pixel_values = torch.cat(
            [gemma_input.pixel_values for gemma_input in gemma_inputs]
        )
        labels = torch.nn.utils.rnn.pad_sequence(
            [gemma_input.labels.squeeze(0) for gemma_input in gemma_inputs],
            batch_first=True,
    ...
    def forward(self, gemma_input: GemmaInput) -> Tensor:
        loss = self.model(
            input_ids=gemma_input.input_ids,
            attention_mask=gemma_input.attention_mask,
            token_type_ids=gemma_input.token_type_ids,
            pixel_values=gemma_input.pixel_values,
            labels=gemma_input.labels,
            return_dict=True,
            use_cache=False,
        ).loss
        return loss

    def generate_inner(self, gemma_input: GemmaInput) -> str:
        ...
        output = self.processor.tokenizer.batch_decode(
            output_ids[:, gemma_input.input_ids.shape[1] :],
